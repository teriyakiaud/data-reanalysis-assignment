...
```
The results of this section show that the model I've named as m2 or NPP.O2 ~ Duration + Potentiometric.pH is the best model to use. THis matches up with the model that the authors of the paper used. Though we had different AIC values, they followed the same trend. 
...
summary(s2)
```
We see here that the best model for respiration in S. latissima is one that takes into account both ocean acidification and time, just like we see in P. palmata for NPP. 
